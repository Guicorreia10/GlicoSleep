...
const TabsLayout = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        // Verificar a sessão de autenticação no Supabase
        const { data: { session } } = await supabase.auth.getSession();

        if (session) {
          setIsAuthenticated(true); // O usuário está autenticado
        } else {
          setIsAuthenticated(false); // Não autenticado, redireciona para SignIn
  ...
  if (!isAuthenticated) {
    // Bloqueia a renderização das Tabs até que o usuário esteja autenticado
    return null;
  }
